...
export const textMask = (value: string) => value.replace(/\b[a-z]/g, (f: string) => f.toUpperCase());
...
export const dateMask = (value: { _isAMomentObject: any; format: (arg0: string) => any; split: (arg0: string) => any[]; replace: (arg0: RegExp, arg1: string) => string; } | null) => {
  if (value != null) {
...
export const reserveDateTimeMask = (value: { _isAMomentObject: any; format: (arg0: string) => any; } | null) => {
  if (value != null) {
    if (value._isAMomentObject) {
...
export const monetaryMask = (value: any, precison = 2) => {
  if (value != null) {
    ...
    return result.toFixed(precison).replace('.', ',').replace(/(\d)(?=(\d{3})+,)/g, '$1.');
  }
  return '0,00';
};
...
export const aplicaMascara = (documento: string, mascara: string) => {
  ...
  const caracteresRegex = new RegExp(caracteresEspeciais.reduce((acc: any, caractere: any) => `${acc}\\${caractere}|`, '').replace(/\|$/, ''), 'g')
...
export const ligaPalavras = (separador = '') => (...palavras: any) => (palavras || []).filter((e: any) => !!e).join(separador);
...
export const textWithoutAccents = (text = '') => {
  return text.toString().normalize('NFD').replace(/\p{Diacritic}/gu, '')
}
